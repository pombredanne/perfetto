...
def CheckTslint(input_api, output_api):
  ...
  # Some tslint rules require type information and thus need the whole project.
  # We therefore call tslint on the whole project instead of only the changed
  # files. It is possible to break tslint on files that was not changed by
  # changing the type of an object.
  if subprocess.call([tslint, '--project', ui_path, '--format', 'codeFrame']):
    return [
