...
class CpuSliceTrack extends TrackImpl {
  ...
  static create(trackConfig: TrackConfig): CpuSliceTrack {
    return new CpuSliceTrack(trackConfig);
  }
  ...
  draw(vCtx: VirtualCanvasContext, width: number, timeScale: TimeScale): void {
    ...
    vCtx.fillStyle = '#000';
    vCtx.fillText(this.trackConfig.name + ' rendered by canvas', shownStart, 60);
  }
}
