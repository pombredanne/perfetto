...
export const Track = {
  oninit({attrs}) {
    ...
    const trackCreator = trackRegistry.getCreator(attrs.trackConfig.type);
    this.trackImpl = trackCreator.create(attrs.trackConfig);
  },
    ...
    return m(
        '.track',
        {
          style: {
            ...
            left: 0,
            width: '100%',
            height: `${attrs.trackConfig.height}px`,
          }
        },
        m(TrackShell,
          {name: attrs.trackConfig.name},
          ...
          m('.marker',
            ...
            attrs.trackConfig.name + ' DOM Content')));
  },
