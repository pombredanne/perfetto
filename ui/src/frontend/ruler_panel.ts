...
export class RulerPanel extends Panel {
  view() {
  ...
  renderCanvas(ctx: CanvasRenderingContext2D, size: PanelSize) {
    ...
    const selectedSpan = globals.state.selectedSpan;
    const hoveredSpan = globals.frontendLocalState.hoveredSpan;

    if (selectedSpan !== null) {
      const start = selectedSpan.startSec;
      const end = selectedSpan.endSec;
      this.renderSpan(ctx, size, new TimeSpan(start, end));
    } else if (hoveredSpan !== null) {
      this.renderSpan(ctx, size, hoveredSpan);
    }
  }
