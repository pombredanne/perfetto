...
TEST(TraceStorageTest, InsertSecondSched) {
  ...
  storage.AddSliceToCpu(cpu, timestamp, 10, pid_1,
                        storage.InternString(kCommProc1, 8));
...
TEST(TraceStorageTest, InsertThirdSched_SameThread) {
  ...
  const auto& timestamps = storage.SlicesForCpu(cpu).start_ns();
  storage.AddSliceToCpu(cpu, timestamp, 1, pid_1,
                        storage.InternString(kCommProc1, 8));

  storage.AddSliceToCpu(cpu, timestamp + 1, 1, pid_1,
                        storage.InternString(kCommProc1, 8));
