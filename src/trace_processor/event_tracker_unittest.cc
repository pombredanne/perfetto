...
TEST_F(EventTrackerTest, MixedEventsValueDelta) {
  uint32_t cpu = 3;
  uint64_t timestamp = 100;
  StringId name_id_cpu = 0;
  StringId name_id_upid = 0;
  UniquePid upid = 12;
  context.event_tracker->PushCounter(timestamp, 1000, name_id_cpu, cpu,
                                     RefType::kCpuId);
  context.event_tracker->PushCounter(timestamp + 1, 0, name_id_upid, upid,
                                     RefType::kUtid);
  context.event_tracker->PushCounter(timestamp + 3, 5000, name_id_cpu, cpu,
                                     RefType::kCpuId);
  context.event_tracker->PushCounter(timestamp + 9, 1, name_id_upid, upid,
                                     RefType::kUtid);
