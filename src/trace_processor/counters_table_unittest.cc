...
TEST_F(CountersTableUnittest, SelectWhereCpu) {
  ...
  context_.storage->mutable_counters()->AddCounter(
      timestamp, 0, 1, freq, 0, 1 /* cpu */, RefType::kCpuId);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 1, 1, 1, freq + 1000, 1000, 1 /* cpu */, RefType::kCpuId);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 2, 1, 1, freq + 2000, 1000, 2 /* cpu */, RefType::kCpuId);
