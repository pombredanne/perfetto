...
TEST_F(CountersTableUnittest, SelectWhereCpu) {
  ...
  context_.storage->mutable_counters()->AddCounter(
      timestamp, 0, 1, freq, 1 /* cpu */, RefType::CPU_ID);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 1, 1, 1, freq + 1000, 1 /* cpu */, RefType::CPU_ID);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 2, 1, 1, freq + 2000, 2 /* cpu */, RefType::CPU_ID);
...
TEST_F(CountersTableUnittest, GroupByFreq) {
  ...
  uint32_t freq = 3000;
  context_.storage->mutable_counters()->AddCounter(
      timestamp, 1, 1, freq, 1 /* cpu */, RefType::CPU_ID);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 1, 2, 1, freq + 1000, 1 /* cpu */, RefType::CPU_ID);
  context_.storage->mutable_counters()->AddCounter(
      timestamp + 3, 0, 1, freq, 1 /* cpu */, RefType::CPU_ID);
