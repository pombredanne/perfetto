...
class TestSysStatsDataSource : public SysStatsDataSource {
 public:
  TestSysStatsDataSource(TracingSessionID id,
                         std::unique_ptr<TraceWriter> writer,
                         const DataSourceConfig& config)
      : SysStatsDataSource(id, std::move(writer), config) {}
...
class SysStatsDataSourceTest : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<TestSysStatsDataSource> GetSysStatsDataSource(
    ...
    return std::unique_ptr<TestSysStatsDataSource>(
        new TestSysStatsDataSource(0, std::move(writer), cfg));
  }
};

TEST_F(SysStatsDataSourceTest, WriteOnceProcess) {
  auto data_source = GetSysStatsDataSource(DataSourceConfig());
...
TEST_F(SysStatsDataSourceTest, DontRescanCachedPIDsAndTIDs) {
