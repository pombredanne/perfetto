...
bool TraceBuffez::MaybePatchChunkContents(ProducerID producer_id,
                                          WriterID writer_id,
                                          ChunkID chunk_id,
                                          const Patch* patches,
                                          size_t patches_size,
  ...
  for (size_t i = 0; i < patches_size; i++) {
    ...
    TRACE_BUFFER_DLOG("PatchChunk {%" PRIu64 ",%" PRIu32
                      ...
                      patch[3]);
    if (off < chunk_begin || off > chunk_end - Patch::kSize) {
    ...
    PERFETTO_DCHECK(memcmp(off, &zero, Patch::kSize) == 0);

    memcpy(off, &patches[i].data[0], Patch::kSize);
  }
  TRACE_BUFFER_DLOG(
