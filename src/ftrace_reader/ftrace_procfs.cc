...
char ReadOneCharFromFile(const std::string& path) {
  base::ScopedFile fd = base::ScopedFile::OpenFile(path.c_str(), O_RDONLY);
  PERFETTO_CHECK(fd);
  char result = '\0';
...
void FtraceProcfs::ClearTrace() {
  ...
  base::ScopedFile fd =
      base::ScopedFile::OpenFile(path.c_str(), O_WRONLY | O_TRUNC);
...
bool FtraceProcfs::WriteToFile(const std::string& path,
  ...
  base::ScopedFile fd = base::ScopedFile::OpenFile(path.c_str(), O_WRONLY);
  if (!fd)
    return false;
...
base::ScopedFile FtraceProcfs::OpenPipeForCpu(size_t cpu) {
  ...
  return base::ScopedFile::OpenFile(path.c_str(), O_RDONLY | O_NONBLOCK);
}
